...
show(io::IO, x::JuliaConSchedule) = print(io, "JuliaCon ", Dates.Year(x.start).value, " Schedule")
show(io::IO, ::MIME"text/plain", x::JuliaConSchedule) = show(io, x)

show(io::IO, x::JuliaConDay) = print(io, "JuliaCon ", Dates.Year(x.start).value, ": Day ", x.number)
